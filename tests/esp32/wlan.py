...
for net in scan_r:
    if len(net.ssid)<1:
        print("Failed ssid check")
    if len(net.bssid)!=6:
        print("Failed bssid check")
    if net.rssi > 0:
        print("Failed rss check")
    if net.channel < 1 or net.channel > 14:
        print("Failed channel check")
    if net.sec < 0:
        print("Failed channel sec")
...
# test init again
wifi.init(mode = WLAN.AP, ssid='www.wipy.io', auth=None, channel=5, antenna=WLAN.INT_ANT)
print(wifi.mode() == WLAN.AP)
