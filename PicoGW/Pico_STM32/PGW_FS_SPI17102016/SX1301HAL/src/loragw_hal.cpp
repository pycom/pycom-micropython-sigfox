...
static bool lgw_is_started=true;
...
int lgw_receive(uint8_t max_pkt, struct lgw_pkt_rx_s *pkt_data) {
    ...
    for (nb_pkt_fetch = 0; nb_pkt_fetch < max_pkt; ++nb_pkt_fetch) {
      ...
      //  DEBUG_PRINTF("FIFO content: %x %x %x %x %x\n", buff[0], buff[1], buff[2], buff[3], buff[4]);
        ...
        DEBUG_PRINTF("rf chain =%d freq =%d rssi = %d]\n", p->rf_chain,p->freq_hz,p->rssi);
        ...
        //   DEBUG_PRINTF(" DataRate %d cr = %d snr = %f snr min = %f snr max = %f \n", sf,cr, p->snr,p->snr_min,p->snr_max); 
            ...
            /* RSSI correction */
            p->rssi = RSSI_FSK_POLY_0 + RSSI_FSK_POLY_1 * p->rssi + RSSI_FSK_POLY_2 *p->rssi*p->rssi; //pow(p->rssi, 2);
        } else {
        ...
        p->count_us = raw_timestamp - timestamp_correction+Sx1301.offtmstpstm32;//pgw offset
        p->crc = (uint16_t)buff[sz+10] + ((uint16_t)buff[sz+11] << 8);

        /* advance packet FIFO */
        lgw_reg_w(LGW_RX_PACKET_DATA_FIFO_NUM_STORED, 0);
    }
    // DEBUG_PRINTF("Receive from 1301 %d packet(s)\n",nb_pkt_fetch); 
    return nb_pkt_fetch;
}
...
int lgw_send(struct lgw_pkt_tx_s pkt_data) {
  ...
  /*  if (lgw_is_started == false) {
    ...
    {
        count_trig = pkt_data.count_us - TX_START_DELAY-Sx1301.offtmstpstm32; //PGW
...
int lgw_txgain_setconf(struct lgw_tx_gain_lut_s *conf) {
    ...
    for (i = 0; i < txgain_lut.size; i++) {
        ...
        txgain_lut.lut[i].rf_power = conf->lut[i].rf_power;
				DEBUG_PRINTF("txgainlut = %d\n",txgain_lut.lut[i].pa_gain); 
    }
  
    return 8;// LGW_HAL_SUCCESS;
}
...
int lgw_start(void) {
    ...
    /* Enable clocks */
	 ...
	 //Sx1301.offtmstpstm32+=Sx1301.timerstm32.read_us()+Sx1301.hosttm;// TBD manage wrap may be done alone !!!!
    lgw_reg_w(LGW_CLK32M_EN, 1);
		lgw_get_trigcnt(&countsx1301);
		//if (timerlatchp==0)
		//{	
	  //Sx1301.offtmstpstm32+=Sx1301.timerstm32.read_us()+Sx1301.hosttm-countsx1301;//+drift;
    //timerlatch = Sx1301.timerstm32ref.read_us()-offset-Sx1301.offtmstpstm32;
		//Sx1301.offtmstpstm32+=timerlatch;
		//}
    //else{
    Sx1301.offtmstpstm32 = Sx1301.timerstm32ref.read_us()-Sx1301.offtmstpstm32ref; 
			if (Sx1301.firsttx==1)
			{Sx1301.offtmstpstm32++; // to manage the round to one ms
			 Sx1301.firsttx=2;
			}
			else
			{
	  ...
	  DEBUG_PRINTF("timer diff %d and driftd %d real drift%d \n",timerlatch,(timerlatch-timerlatchp),drift);
    	if (timerlatchp!=0)
	  	{drift=drift+timerlatch-timerlatchp;}	
			...
			{offset=timerlatch; } // to compensate offset beetwen the 2 counters
    ...
    lgw_reg_w(LGW_GPIO_SELECT_OUTPUT,0);
		lgw_get_trigcnt(&countsx1301);
	// Sx1301.offtmstpstm32+=Sx1301.timerstm32.read_us()+Sx1301.hosttm-countsx1301;
	 //pc.printf("counter 1301 = %d\n",countsx1301);
	//  lgw_reg_w(LGW_RADIO_A_EN,1);
  // lgw_reg_w(LGW_RADIO_B_EN,1);
   // Sx1301.offtmstpstm32+=Sx1301.timerstm32.read_us()+Sx1301.hosttm;// TBD manage wrap may be done alone !!!!
	//	lgw_reg_w(LGW_CLKHS_EN, 1);
		//	pc.printf("il est 9heureeees = %d  et %d\n",Sx1301.offtmstpstm32,Sx1301.timerstm32.read_us());	
	//	 Sx1301.offtmstpstm32=0;
    /* GPIOs table :
    DGPIO0 -> N/A
    DGPIO1 -> N/A
    DGPIO2 -> N/A
    ...
    */
    ...
    /* Load firmware */
		load_firmware(MCU_ARB, MCU_ARB_FW_BYTE);
    load_firmware(MCU_AGC,MCU_AGC_FW_BYTE);
...
//    wait_ms(1);
    ...
    for (i = 0; i < txgain_lut.size; ++i) {
		    //pc.printf("ERROR:tx hain lut size = %d lut = %d\n",txgain_lut.size, txgain_lut.lut[i].dac_gain);
    ...
    /* */
 //tbc for V2 comment the next line
//	lgw_reg_w(LGW_RADIO_A_EN,1);
	 #ifdef V2
		//	lgw_reg_w(LGW_RADIO_A_EN,1);
		#else
