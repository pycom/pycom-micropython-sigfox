            ...
            if (xQueueReceive(xCmdQueue, &cmd_data, 0)) {
                ...
                case E_LORA_CMD_JOIN:
                    ...
                    if (lora_obj.activation == E_LORA_ACTIVATION_OTAA) {
                        ...
                        lora_obj.otaa.data_rate = cmd_data.info.join.otaa.data_rate;
        ...
        case E_LORA_STATE_JOIN:
            ...
            if (!lora_obj.joined) {
                if (lora_obj.activation == E_LORA_ACTIVATION_OTAA) {
                    ...
                    mlmeReq.Req.Join.data_rate = (uint8_t) lora_obj.otaa.data_rate;
...
STATIC mp_obj_t lora_join(mp_uint_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    ...
    int32_t data_rate = LORAMAC_RX_MAX_DATARATE + 1;

    // get the data rate
    if (args[2].u_obj != mp_const_none) {
    	if (lora_validate_data_rate(mp_obj_get_int(args[2].u_obj)))
    	{
    		data_rate = mp_obj_get_int(args[2].u_obj);
    	}
    }

	cmd_data.info.join.otaa.data_rate = data_rate;
