...
STATIC mp_obj_t os_mount(mp_uint_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    ...
    if (pathlen < 2 || path[0] !='/' || strchr(&path[1], '/')) {
        nlr_raise(mp_obj_new_exception_msg_varg(&mp_type_OSError, "invalid mount point %s", path));
    }
...
mount_error:
    nlr_raise(mp_obj_new_exception_msg_varg(&mp_type_OSError, "%s cannot be unmounted", path));
}
...
STATIC mp_obj_t os_mkfs(mp_obj_t device) {
    ...
    if (MP_OBJ_IS_STR_OR_BYTES(device)) {
        ...
        if (path[0] != '/') {
            nlr_raise(mp_obj_new_exception_msg_varg(&mp_type_OSError, "%s is not an absolute path", path));
        }
    } else {
        // mount it briefly
    ...
    if (res != FR_OK) {
        nlr_raise(mp_obj_new_exception_msg_varg(&mp_type_OSError, "error while attempting to format %s", path));
    }
    return mp_const_none;
}
