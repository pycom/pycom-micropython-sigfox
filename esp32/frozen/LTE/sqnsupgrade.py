...
class sqnsupgrade:
    ...
    def recover_ext(self, port, mfile, debug=False, pkgdebug=False):
        print('<<< Welcome to the SQN3330 recovery firmware updater >>>')
        success = self.__run(atneg_only = True, direct= False, port=port, debug=debug, pkgdebug=pkgdebug)
        #success = True
        if debug: print('Success1? {}'.format(success))
        if success:
            success = False
            success = self.__run(file_path=mfile, load_ffh=True, direct=True, port=port, debug=debug, pkgdebug=pkgdebug)
            if debug: print('Success2? {}'.format(success))
            if success:
                self.__run(bootrom=True, switch_ffh=True, resume=True, direct=False, port=port, debug=debug, pkgdebug=pkgdebug)
            else:
                print('Unable to load updater from {}'.format(mfile))
        else:
            print('Failed to perform AT auto-negotiation')
...
if 'FiPy' in sysname or 'GPy' in sysname:
    ...
    def recover(port, mfile, debug=False):
        sqnup = sqnsupgrade()
        sqnup.recover_ext(port, mfile, debug, False)
