...
IRAM_ATTR mp_irq_obj_t *mp_irq_find (mp_obj_t parent) {
...
IRAM_ATTR void mp_irq_handler (mp_obj_t self_in) {
    mp_irq_obj_t *self = self_in;
    if (self) {
        if (self->pyhandler && self->handler != mp_const_none) {
            ...
            if (nlr_push(&nlr) == 0) {
                ...
                mp_printf(&mp_plat_print, "uncaught exception inside irq handler\n");
                mp_obj_print_exception(&mp_plat_print, (mp_obj_t)nlr.ret_val);
                mperror_signal_error();
            }
            gc_unlock();
        } else if (self->handler) {
            ((void(*)(void))self->handler)();
        }
    }
}
