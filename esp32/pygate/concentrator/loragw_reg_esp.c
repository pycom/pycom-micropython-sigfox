...
/* soft-reset function */
int esp_lgw_soft_reset(void) {
    sx1308_spiWrite(0, 0x80); /* 1 -> SOFT_RESET bit */
...
int esp_reg_w_align32(void *spi_target, uint8_t spi_mux_mode, uint8_t spi_mux_target, const struct lgw_reg_s *r, int32_t reg_value) {
    int i, size_byte;
    uint8_t buf[4] = {0, 0, 0, 0};

    if ((r->leng == 8) && (r->offs == 0)) {
        /* direct write */
        sx1308_spiWrite(r->addr, (uint8_t)reg_value);
    } else if ((r->offs + r->leng) <= 8) {
        /* single-byte read-modify-write, offs:[0-7], leng:[1-7] */
        buf[0] = sx1308_spiRead(r->addr);
        ...
        sx1308_spiWrite(r->addr, (uint8_t)buf[3]);
        ...
        sx1308_spiWriteBurst( r->addr, buf, size_byte);
    } else {
...
int esp_reg_r_align32(void *spi_target, uint8_t spi_mux_mode, uint8_t spi_mux_target, const struct lgw_reg_s *r, int32_t *reg_value) {
    ...
    if ((r->offs + r->leng) <= 8) {
        ...
        size_byte = (r->leng + 7) / 8; /* add a byte if it's not an exact multiple of 8 */
        sx1308_spiReadBurst(r->addr, bufu, size_byte);
        u = 0;
...
int esp_lgw_connect(void) {
    sx1308_init();
    sx1308_spiWrite(LGW_PAGE_REG, 0);
...
int esp_lgw_reg_w(uint16_t register_id, int32_t reg_value) {
    const struct lgw_reg_s *r;
    ...
    esp_reg_w_align32(0, 0, 0, r, reg_value);
...
int esp_lgw_reg_r(uint16_t register_id, int32_t *reg_value) {
    const struct lgw_reg_s *r;
    ...
    esp_reg_r_align32(0, 0, 0, r, reg_value);
...
int esp_lgw_reg_wb(uint16_t register_id, uint8_t *data, uint16_t size) {
    const struct lgw_reg_s *r;
    ...
    /* do the burst write */
    sx1308_spiWriteBurst(r->addr, data, size);
...
int esp_lgw_reg_rb(uint16_t register_id, uint8_t *data, uint16_t size) {
    const struct lgw_reg_s *r;
    ...
    /* do the burst read */
    sx1308_spiReadBurst(r->addr, data, size);
