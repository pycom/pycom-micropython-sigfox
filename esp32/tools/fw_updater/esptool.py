...
class ESPLoader(object):
    ...
    @stub_and_esp32_function_only
    def change_baud(self, baud, resultUpdateList = None):
        print("Changing baud rate to %d" % baud)
        self.command(self.ESP_CHANGE_BAUDRATE, struct.pack('<II', baud, 0))
        print("Changed.")
        if resultUpdateList is not None:
            resultUpdateList.append("Changed baud rate to %d" % baud)
        self._set_port_baudrate(baud)
    ...
    @stub_function_only
    def erase_region(self, offset, size, progress_fs=None):
        ...
        if progress_fs is not None:
            try:
                ...
                progress_fs('Erasing %s at 0x%08X' % (humanfriendly.format_size(size, binary=True), offset))
            except:
                progress_fs('Erasing %d byte at 0x%08X' % (size, offset))
        self.check_command("erase region", self.ESP_ERASE_REGION, struct.pack('<II', offset, size))

    @stub_function_only
    def read_flash(self, offset, length, progress_fn=None, resultUpdateList=None, partition = None):
        ...
        if resultUpdateList is not None:
                try:
                    ...
                    if partition is not None:
                        resultUpdateList.append('Read %s from %s in %s' % (humanfriendly.format_size(len(data), binary=True), partition, humanfriendly.format_timespan(t)))
                    else:
                        resultUpdateList.append('Read %s at 0x%08X in %s' % (humanfriendly.format_size(len(data), binary=True), offset, humanfriendly.format_timespan(t)))
                except Exception:
                    if partition is not None:
                        resultUpdateList.append('Read %d bytes from %s in %.1f seconds...' % (len(data), partition, t))
                    else:
                        resultUpdateList.append('Read %d bytes at 0x%08X in %.1f seconds...' % (len(data), offset, t))
...
def write_flash(esp, args, ui_label=None, file_name=None, resultUpdateList=None, progress_fs=None, verbose=True, partition=None):
    ...
    for address, argfile in args.addr_filename:
        ...
        while len(image) > 0:
            if file_name is not None:
                msg = 'Writing %s at 0x%08X... (%d %%)' % (file_name, address + seq * esp.FLASH_WRITE_SIZE, 100 * (seq + 1) // blocks)
            else:
                msg = 'Writing at 0x%08X... (%d %%)' % (address + seq * esp.FLASH_WRITE_SIZE, 100 * (seq + 1) // blocks)
            if ui_label is not None:
                    ui_label.setText('\r{}'.format(msg))
            elif progress_fs is not None:
                progress_fs(msg)
            else:
        ...
        if args.compress:
            if t > 0.0:
                speed_msg = " (%.1f kbit/s)" % (uncsize / t * 8 / 1000)
            print('\rWrote %d bytes (%d compressed) at 0x%08x in %.1f seconds%s...\n' % (uncsize, written, address, t, speed_msg))
            try:
                import humanfriendly
                if file_name is None and partition is not None:
                    ui_msg = 'Wrote %s to %s in %s' % (humanfriendly.format_size(uncsize, binary=True), partition, humanfriendly.format_timespan(t))
                else:
                    ui_msg = 'Wrote %s from %s in %s' % (humanfriendly.format_size(uncsize, binary=True), file_name, humanfriendly.format_timespan(t))
            except Exception:
                if file_name is not None:
                    ui_msg = 'Wrote %d bytes from %s in %.1f seconds' % (uncsize, file_name, t)
                elif partition is not None:
                    ui_msg = 'Wrote %d bytes to %s in %.1f seconds' % (uncsize, partition, t)
                else:
                    ui_msg = 'Wrote %d bytes in %.1f seconds' % (uncsize, t)
            if resultUpdateList is not None:
                resultUpdateList.append(ui_msg)
        else:
            if t > 0.0:
                speed_msg = " (%.1f kbit/s)" % (written / t * 8 / 1000)
            print('\rWrote %d bytes at 0x%08x in %.1f seconds%s...' % (written, address, t, speed_msg))
            if resultUpdateList is not None:
                resultUpdateList.append()
