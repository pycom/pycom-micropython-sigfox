...
if ! [ $0 = "tools/makepkg.sh" ]; then echo "Need to run as tools/makepkg.sh!"; exit 1; fi
...
tar -czf ../../../${FILE_NAME} bootloader.bin partitions.bin ${BOARD_NAME_L}.bin script
cd ..
echo "Release package build/${FILE_NAME} created successfully."
