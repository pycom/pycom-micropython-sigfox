...
*/
#include "lora/mac/secure-element.h"

#include <stdlib.h>
#include <stdint.h>

#include "lora/mac/LoRaMacCrypto.h"
#include "utilities.h"
#include "lora/system/crypto/aes.h"
...
SecureElementStatus_t SecureElementAesEncrypt( uint8_t* buffer, uint16_t size, KeyIdentifier_t keyID, uint8_t* encBuffer )
{
    ...
    if( retval == SECURE_ELEMENT_SUCCESS )
    {
        aes_set_key_lora( pItem->KeyValue, 16, &SeNvmCtx.AesContext );

        uint8_t block = 0;

        while( size != 0 )
        {
            aes_encrypt_lora( &buffer[block], &encBuffer[block], &SeNvmCtx.AesContext );
            block = block + 16;
            size = size - 16;
        }
    }
