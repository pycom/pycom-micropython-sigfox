 ...
 *          the frame]
 */
//static bool ValidatePayloadLength( uint8_t lenN, int8_t datarate, uint8_t fOptsLen ); Pycom: Changed to non-static
...
static void ProcessRadioRxDone( void )
{
    ...
    // Pycom: Setting TimeStamp
    MacCtx.McpsIndication.TimeStamp = RxDoneParams.LastRxDone;
    Radio.Sleep( );
...
static void ResetMacParameters( void )
{
    ...
    // Pycom: Not resetting the Default Regions since we might have manually updated them before Join and this would reset the changes
...
LoRaMacStatus_t LoRaMacMlmeRequest( MlmeReq_t* mlmeRequest )
{
    ...
    switch( mlmeRequest->Type )
    {
        case MLME_JOIN:
        {
            ...
            // Pycom: Add Join DR selection as and when the region permits. Probably not required because AlternateDR() now just returns CurrentDR in the current version
...
{
    void *modCtx = NULL;
    void *secElemCtx = NULL;

    size_t size;
    GetNvmCtxParams_t params;

    modlora_nvs_set_blob(E_LORA_NVS_ELE_LORAMAC_CTX, &NvmMacCtx, sizeof(NvmMacCtx));

    modCtx = RegionGetNvmCtx(NvmMacCtx.Region, &params);
    modlora_nvs_set_blob(E_LORA_NVS_ELE_REGION_CTX, &modCtx, params.nvmCtxSize);

    modCtx = LoRaMacCommandsGetNvmCtx( &size );
    modlora_nvs_set_blob(E_LORA_NVS_ELE_MAC_CMD_CTX, &modCtx, size);

    modCtx = SecureElementGetNvmCtx( &size );
    modlora_nvs_set_blob(E_LORA_NVS_ELE_SEC_ELEM_CTX, &secElemCtx, size);
}
