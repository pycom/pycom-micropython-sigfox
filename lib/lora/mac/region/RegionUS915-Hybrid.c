...
IRAM_ATTR void RegionUS915HybridSetBandTxDone( SetBandTxDoneParams_t* txDone )
{
...
LoRaMacStatus_t RegionUS915HybridChannelManualAdd( ChannelAddParams_t* channelAdd )
{
    ...
    if( id >= US915_HYBRID_MAX_NB_CHANNELS )
    {
        return LORAMAC_STATUS_PARAMETER_INVALID;
    }
    ...
    if( channelAdd->NewChannel->DrRange.Fields.Max != US915_HYBRID_TX_MAX_DATARATE )
    {
        drInvalid = true;
    }
...
bool RegionUS915HybridChannelsManualRemove( ChannelRemoveParams_t* channelRemove  )
{
    ...
    if ( !RegionCommonChanDisable( ChannelsMask, id, US915_HYBRID_MAX_NB_CHANNELS ) ) {
        return false;
    }
