...
To use the REPL, you must connect to the WiPy either via :ref:`telnet <pycom_telnet>`,
...
to the board. If you are connected to the WiPy via WiFi then use the IP address as `device` (192.168.4.1)
...
and enter the IP address of your WiPy (192.168.4.1 when in ``WLAN.AP`` mode),
then click open.
...
at the MicroPython prompt, which performs a soft reset.  You will see a message something like::
    ...
    >>>
    PYB: soft reboot
    MicroPython v1.4.6-146-g1d8b5e5 on 2016-10-21; WiPy with ESP32
