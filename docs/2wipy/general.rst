General information about the WiPy 2.0
======================================
...
When the WiPy boots with the default factory configuration, it' starts in Access Point mode',
and it's ``ssid`` that starts with: ``WiPy-wlan`` and ``key: www.pycom.io``.
Connect to this network and the WiPy will be reachable at ``192.168.4.1``. In order
...
other tools like Putty are also compatible. The default credentials are: **user:** ``micro``, **password:** ``python``.
...
Upgrading with the WiPy updater stand-alone tool
------------------------------------------------

1. Download the stand-alone tool from: https://www.pycom.io/support/supportdownloads/
2. Boot your WiPy in safe mode.
3. Connect to the ``WiPy-wlan`` WLAN network (password: ``www.pycom.io``)
...
the LED will start blinking a bit faster and the WiPy will select the previous
...
quickly for 1.5 seconds and the WiPy will proceed to boot.
